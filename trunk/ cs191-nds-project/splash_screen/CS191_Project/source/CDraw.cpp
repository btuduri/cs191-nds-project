...
{
	buffer[SCREEN_WIDTH*p.y + p.x] = RGB15(c.getR(),  c.getG(), c.getB()) | BIT(15);
}
...
void CDraw::drawLine(u16* buffer, CPoint a, CPoint b, CColor c, s16 weight=0)
{
	...
	if( xdif == 0 )  //vertical line
	{
		...
		{
			buffer[SCREEN_WIDTH*i + a.x] = RGB15(c.getR(), c.getG(), c.getB()) | BIT(15);
		}
	}
	else if( ydif == 0) //horizontal line
	{
		...
		{
			buffer[SCREEN_WIDTH*a.y + i] = RGB15(c.getR(), c.getG(), c.getB()) | BIT(15);
		}		
	}
	else if( yd > xd ) //if the yd is greater than xd, then we'll have to draw more pixels on the Y axis than the X.
	{
		...
		{
			x = (int)round((i-lb)/lm);
			buffer[SCREEN_WIDTH*i + x] = RGB15(c.getR(), c.getG(), c.getB()) | BIT(15);
		}
	}
	else //if xd is greater, then we'll have to draw more pixels on the X axis. This also draws diagonals where DX=DY
	{
		...
		{
			y = (int)round((lm*i) + lb);
			buffer[SCREEN_WIDTH*y + i] = RGB15(c.getR(),  c.getG(), c.getB()) | BIT(15);
		}
	}
}
...
void CDraw::drawCircle(u16* buffer, CPoint a, int r, CColor c, bool filled = 0)
{
	...
	*/ 
	...
	for( i = (a.x - r); i < (a.x + r); i++ )
	{
		...
		buffer[SCREEN_WIDTH*p + i] = RGB15(c.getR(), c.getG(), c.getB()) | BIT(15);
		q = abs((int)round(-sqrt( r2 - (double)((i - a.x)*(i - a.x))) + a.y));
		buffer[SCREEN_WIDTH*q + i] = RGB15(c.getR(), c.getG(), c.getB()) | BIT(15);
		if( filled )
		{
	...
	for( i = (a.y - r); i < (a.y + r); i++ )
	{
		...
		buffer[SCREEN_WIDTH*i + p] = RGB15(c.getR(), c.getG(), c.getB()) | BIT(15);
		q = abs((int)round(-sqrt( r2 - (double)((i - a.y)*(i - a.y))) + a.x));
		buffer[SCREEN_WIDTH*i + q] = RGB15(c.getR(), c.getG(), c.getB()) | BIT(15);	
		if( filled )
		{
			if( q < 0 )
				q = 0;
