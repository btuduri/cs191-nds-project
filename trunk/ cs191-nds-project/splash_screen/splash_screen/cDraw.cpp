...
void drawLine(u16* buffer, Point a, Point b, Color c, s16 weight=0)
{
	...
	{
		int x;
		swap_points(&a, &b, 2);	// we want to draw from the "lower" point to the higher.
...
void drawRectangle(u16* buffer, Point a, Point b, Color c, bool filled=0)
{
	...
	if( filled )
	{
		for(int i=a.y; i<b.y; i++)
		{
			drawLine(buffer, Point(a.x, i), Point(b.x, i), c);
		}
	}
	else
	{
		drawLine(buffer, a, Point(b.x, a.y), c);	// top side of rectangle
		drawLine(buffer, a, Point(a.x, b.y), c);	// left side of rectangle
		drawLine(buffer, Point(a.x, b.y), b, c);	// bottom side of rectangle
		drawLine(buffer, Point(b.x, a.y), b, c); 	// right side of rectangle
	}
}
